...
bot.action(/^trial_server_ssh_(\d+)$/, async (ctx) => {
  ...
  try {
    ...
    let apiRes;
    try {
      apiRes = await axios.get(url, { timeout: 10000 });
    } catch (e) {
    ...
    // --- Mapping data dari API (support multiple shapes) ---
    const d = apiRes.data.data || apiRes.data || {};

    const username   = d.username || d.user || null;
    const password   = d.password || d.pass || d.p || null;
    const expired    = d.expiration || d.exp || d.expired || d.expires || null;
    const ip_limit   = d.ip_limit || d.limitip || d.limit_ip || d.iplimit || null;
    const ns_domain  = d.ns_domain || d.ns || null;
    const city       = d.city || d.kota || null;
    const pubkey     = d.public_key || d.publicKey || d.pubkey || null;
...
Save Acc : https://${domain}:81/ssh-${username || 'account'}.txt

📆 *Expired:* ${expired || 'N/A'}
🛡️ *Limit IP:* \`${ip_limit || 'N/A'}\`
...
bot.action(/^trial_server_vmess_(\d+)$/, async (ctx) => {
  ...
  try {
    ...
    // Mapping using fields produced by your vmess script
    const d = apiRes.data.data || apiRes.data; // support both shapes

    const username   = d.username || null;
    const uuid       = d.uuid || null;
    const ns_domain  = d.ns_domain || d.ns || null;
    const city       = d.city || null;
    const public_key = d.public_key || null;
    const expiration = d.expiration || d.exp || null;
    const link_tls   = d.link_tls || d.link_tls || null;
    const link_ntls  = d.link_ntls || d.link_ntls || null;
    const link_grpc  = d.link_grpc || d.link_grpc || null;
...
bot.action(/^trial_server_vless_(\d+)$/, async (ctx) => {
  ...
  try {
    ...
    const public_key = d.public_key || null;
    const expiration = d.expiration || d.exp || null;
    const link_tls   = d.link_tls || null;
    const link_ntls  = d.link_ntls || null;
    const link_grpc  = d.link_grpc || null;
...
bot.action(/^trial_server_trojan_(\d+)$/, async (ctx) => {
  ...
  try {
    ...
    const username   = d.username || null;
    const uuid       = d.uuid || d.password || null;   // trojan uses uuid as password
    const ns_domain  = d.ns_domain || d.ns || null;
    const city       = d.city || null;
    const public_key = d.public_key || null;
    const expiration = d.expiration || d.exp || null;
...
bot.action(/^trial_server_shadowsocks_(\d+)$/, async (ctx) => {
  ...
  try {
    ...
    const username   = d.username || null;
    const password   = d.password || d.uuid || null;
    const method     = d.method || null;
    const ns_domain  = d.ns_domain || d.ns || null;
    const city       = d.city || null;
    const public_key = d.public_key || null;
    const expiration = d.expiration || d.exp || null;
    const link_ws    = d.link_ws || d.ss_link_ws || null;
    const link_grpc  = d.link_grpc || d.ss_link_grpc || null;
