...
• Setiap user hanya dapat mengambil <b>1x trial per hari</b>
...
bot.action(/^trial_server_ssh_(\d+)$/, async (ctx) => {
  ...
  try {
    ...
    const [userRow, server] = await Promise.all([
    ...
    let trialCount = userRow?.trial_count_today ?? 0;
    const last = userRow?.last_trial_date ?? null;
    ...
    try {
      ...
      console.error('Error connecting to trial API:', err?.message);
...
👤 User: ${mention}
📩 Trial by: <b>${roleLabel}</b> | ${trialCount + 1} dari ${maxShow}
...
bot.action(/^trial_server_vmess_(\d+)$/, async (ctx) => {
  ...
  try {
    ...
    try {
      ...
      console.error('Error call trialvmess API:', e?.message);
...
🕒 Waktu: <b>${new Date().toLocaleString('id-ID',{timeZone:'Asia/Jakarta'})}</b>
      ...
      try { await bot.telegram.sendMessage(GROUP_ID, notif, { parse_mode: 'HTML' }); }
      catch (e) { console.warn('Gagal kirim notif VMESS:', e.message); }
    }
...
bot.action(/^trial_server_vless_(\d+)$/, async (ctx) => {
  ...
  try {
    ...
    // === TAMPILAN MIRIP CREATEVLESS (KOTAK) ===
    const replyText = `
🌟 *AKUN VLESS TRIAL* 🌟
...
bot.action(/^trial_server_trojan_(\d+)$/, async (ctx) => {
  ...
  try {
    ...
    // mapping fields (sesuaikan dengan output createtrojan)
    const username       = d.username || '-';
    const domainOut      = d.domain || domain || '-';
    ...
    const uuid_or_pass   = d.uuid || d.password || '-'; // trojan uses uuid as password
    const expired        = d.expired || d.expiration || d.exp || 'N/A';
    const quota          = d.quota || d.quota_gb || '0 GB';
    ...
    // === BUILD MESSAGE (mirror createtrojan style) ===
    const msg = `
🌟 *AKUN TROJAN TRIAL* 🌟
...
bot.action(/^trial_server_shadowsocks_(\d+)$/, async (ctx) => {
  ...
  try {
    ...
    // === Mirror createshadowsocks message ===
    const msg = `
...
`.trim();
    ...
    // === NOTIF GRUP — tetap (tidak diubah logikanya) ===
    if (GROUP_ID) {
      const notif = `
<blockquote>
