...
<b>BONUS SPESIAL JUMAT</b>
Setiap hari Jumat kamu bisa klaim
bonus saldo gratis.
Cukup tekan tombol “Klaim Bonus Jumat”
di menu utama. Jangan sampai terlewat!
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
${topResellerText ? `
<b>🏆 TOP CREATORS (7 HARI)</b>
...
// 🔥 Tambahan tombol klaim bonus Jumat
keyboard.push([
  { text: '🎁 Klaim Bonus Jumat', callback_data: 'claim_weekly_bonus' }
]);
...
///bonus
const BONUS_AMOUNT = 1000; // ubah kalau perlu (dalam rupiah)

bot.action('claim_weekly_bonus', async (ctx) => {
  const fromId = ctx.from?.id;
  if (!fromId) {
    return ctx.answerCbQuery('Gagal: user id tidak ditemukan.', { show_alert: true });
  }

  try {
    // Gunakan timezone Asia/Jakarta untuk konsistensi dengan server/user
    const nowJakarta = new Date(new Date().toLocaleString('en-US', { timeZone: 'Asia/Jakarta' }));
    const isFriday = nowJakarta.getDay() === 5; // 5 = Jumat (0 = Minggu)
    if (!isFriday) {
      return ctx.answerCbQuery('Bonus cuma bisa diklaim hari Jumat!', { show_alert: true });
    }
    ...
    const today = `${yyyy}-${mm}-${dd}`; // key unik per hari

    // 1) cek apakah sudah klaim hari ini
    const existing = await dbGetAsync('SELECT * FROM weekly_bonus_claims WHERE user_id = ? AND claimed_date = ?', [fromId, today]);
    if (existing) {
      return ctx.answerCbQuery('Kamu sudah klaim bonus hari ini.', { show_alert: true });
    }

    // 2) lakukan klaim atomik: BEGIN -> UPDATE users -> INSERT transactions -> INSERT weekly_bonus_claims -> COMMIT
    await new Promise((resolve, reject) => {
      db.serialize(async () => {
        db.run('BEGIN TRANSACTION', async (bErr) => {
          if (bErr) return reject(bErr);

          // update saldo
          db.run('UPDATE users SET saldo = saldo + ? WHERE user_id = ?', [BONUS_AMOUNT, fromId], function (uErr) {
            if (uErr) {
              db.run('ROLLBACK', () => {});
              return reject(uErr);
            }

            // insert ke transactions (cek apakah tabel ada di DB mu)
            const ref = `weekly-bonus-${Date.now()}`;
            db.run('INSERT INTO transactions (user_id, amount, type, reference_id, timestamp) VALUES (?, ?, ?, ?, ?)',
              [fromId, BONUS_AMOUNT, 'bonus', ref, Date.now()], function (tErr) {
                if (tErr) {
                  db.run('ROLLBACK', () => {});
                  return reject(tErr);
                }
                ...
                // insert klaim
                db.run('INSERT INTO weekly_bonus_claims (user_id, amount, claimed_date, reference) VALUES (?, ?, ?, ?)',
                  [fromId, BONUS_AMOUNT, today, ref], function (cErr) {
                    if (cErr) {
                      db.run('ROLLBACK', () => {});
                      // handle duplikasi (race) gracefully
                      if (cErr.message && cErr.message.includes('UNIQUE')) {
                        return reject(new Error('ALREADY_CLAIMED'));
                      }
                      return reject(cErr);
                    }
    ...
    // 3) sukses -> beri feedback ke user
    await ctx.answerCbQuery(`✅ Bonus Rp${BONUS_AMOUNT} berhasil diklaim!`, { show_alert: false });

    // opsional: kirim pesan personal sebagai konfirmasi
    try {
      await bot.telegram.sendMessage(fromId, `🎉 Kamu menerima Bonus Rp${BONUS_AMOUNT}. Saldo telah diperbarui.`);
    } catch (sendErr) {
      // kirim pesan bisa gagal jika user blocking; jangan crash
      console.warn('Gagal kirim pesan konfirmasi klaim bonus:', sendErr?.message || sendErr);
    }
  } catch (err) {
    if (err.message === 'ALREADY_CLAIMED') {
      return ctx.answerCbQuery('Bonus sudah diklaim (konkurensi).', { show_alert: true });
    }
    console.error('Error saat klaim bonus:', err);
    try { await ctx.answerCbQuery('Terjadi error, coba lagi nanti.', { show_alert: true }); } catch(e) {}
  }
});
///Upgrade to reseller
...
bot.action(/^trial_server_ssh_(\d+)$/, async (ctx) => {
  ...
  try {
    ...
    exec(`./scripts/trialssh.sh ${serverId}`, async (error, stdout, stderr) => {
      ...
      // ——— Ambil JSON dari stdout (akurat seperti VMESS)
      let json;
      try {
      ...
      try {
        // Field yang diharapkan dari script
        const {
          username, password, ip, domain, city, public_key = 'Public key not available',
          expiration,
          ports = {},
          openvpn_link = '',
          save_link = '',
        ...
        const p = {
          ...
          udp_ssh: ports.udp_ssh ?? '1-65535',
          dns: ports.dns ?? '53',
          ssh_ws: ports.ssh_ws ?? '80, 8080',
          ssh_ssl_ws: ports.ssh_ssl_ws ?? '443',
          ssl_tls: ports.ssl_tls ?? '443',
          ovpn_tcp: ports.ovpn_tcp ?? '1194',
          ovpn_udp: ports.ovpn_udp ?? '2200',
          ovpn_ssl: ports.ovpn_ssl ?? '443',
          badvpn: ports.badvpn ?? '7100, 7300'
        };
        ...
        // ——— Balasan ke user (Markdown, gaya VMESS)
        const replyText = `
⚡ *AKUN SSH TRIAL*
...
🔌 *PORT*
OpenSSH ${p.openssh} | Dropbear ${p.dropbear} | UDP SSH ${p.udp_ssh}
DNS ${p.dns} | WS ${p.ssh_ws} | SSL WS ${p.ssh_ssl_ws} | TLS ${p.ssl_tls}
OVPN TCP ${p.ovpn_tcp} | UDP ${p.ovpn_udp} | SSL ${p.ovpn_ssl} | BadVPN ${p.badvpn}
...
`.trim();
        ...
        // ——— Notif grup (HTML). Kalau gagal, JANGAN kirim error ke user.
        try {
          if (GROUP_ID) {
            const notif = `
<blockquote>
...
🕒 Waktu: <b>${new Date().toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' })}</b>
          ...
          // jangan kirim pesan error kedua ke user
        }
      } catch (e) {
        logger.error('❌ Gagal olah & kirim akun trial SSH:', e.message);
