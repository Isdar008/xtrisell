...
// 🔰 Kirim Menu Utama (HTML Version + Statistik Rapi)
async function sendMainMenu(ctx) {
  ...
  let topResellerText = '';
  let userHari = 0, userMinggu = 0, userBulan = 0, userTotal = 0;
  let globalHari = 0, globalMinggu = 0, globalBulan = 0, globalTotal = 0;
  ...
  try {
    ...
    const userBulanData = await dbGetAsync('SELECT COUNT(*) AS total FROM invoice_log WHERE user_id = ? AND strftime("%Y-%m", created_at) = strftime("%Y-%m", "now")', [userId]);
    ...
    const medals = ['🥇', '🥈', '🥉'];
    topResellerText = `<b>🏆 Top Reseller Mingguan :</b>\n`;
    for (let i = 0; i < 3; i++) {
      const r = topReseller[i];
      if (r) {
        const mention = r.username ? `@${escapeHTML(r.username)}` : `ID_${escapeHTML(r.reseller_id)}`;
        const totalAkun = r.total_create || 0;
        topResellerText += `╟${medals[i]} ${mention} - ${totalAkun} akun\n`;
      } else {
        topResellerText += `╟${medals[i]} - - 0 akun\n`;
      }
    }
    ...
    const globalBulanData = await dbGetAsync('SELECT COUNT(*) AS total FROM invoice_log WHERE strftime("%Y-%m", created_at) = strftime("%Y-%m", "now")');
    ...
    logger.error(`❌ Gagal ambil data user/top reseller/statistik global: ${err.message}`);
  }
...
// Tombol Admin & Reseller jadi satu baris jika perlu
const topButtons = [];
if (role === 'reseller') {
  topButtons.push({ text: '⚙️ Menu Reseller', callback_data: 'menu_reseller' });
}
if (role === 'admin' || adminIds.includes(String(userId))) {
  topButtons.push({ text: '🛠 Menu Admin', callback_data: 'menu_adminreseller' });
}
...
keyboard.push([
  ...
  const text = `
╔════════════════════════╗
╟📂 <b>BOT PANEL VPN AUTOMATIC</b>
╟════════════════════════╝
╟📋 <b>Informasi Akun</b>
╟🛍 <b>Store</b>     : ${escapeHTML(NAMA_STORE)}
╟💳 <b>Saldo</b>     : Rp${saldo.toLocaleString('id-ID')}
╟📜 <b>Total Akun Dibuat</b> : ${userTotal}
╟🏷 <b>Status</b>    : ${escapeHTML(roleLabel)}
╟🆔 <b>ID Anda</b>   : <code>${userId}</code>
╟🔒 <b>Admin Bot</b> : @joyhayabuse
╟🕒 <b>Update Cache</b> : ${new Date(cacheStatus.lastUpdated).toLocaleTimeString('id-ID')}
╟════════════════════════╗
╟<b>📊 Statistik Global Akun :</b>
╟📍 Hari ini    : ${globalHari} akun
╟📍 Minggu ini  : ${globalMinggu} akun
╟📍 Bulan ini   : ${globalBulan} akun
╟📍 Total       : ${globalTotal} akun
╟════════════════════════╝
╟${topResellerText}
`.trim();
...
// ==========================
// 🔻 PILIH USER UNTUK KURANGI SALDO (PAGINATION READY)
// ==========================
const reduceSaldoState = {}; // state per admin untuk input angka

bot.action('reducesaldo_user', async (ctx) => {
  try {
    logger.info('Kurangi saldo user process started');
    await ctx.answerCbQuery();
    const currentPage = 0;
    await sendReduceSaldoPage(ctx, currentPage);
  } catch (error) {
    logger.error('❌ Gagal memulai proses pengurangan saldo:', error);
    await ctx.reply(`❌ *${error}*`, { parse_mode: 'Markdown' });
  }
});

// Fungsi bantu kirim halaman user
async function sendReduceSaldoPage(ctx, page) {
  const offset = page * 20;

  const users = await new Promise((resolve, reject) => {
    db.all('SELECT id, user_id FROM Users LIMIT 20 OFFSET ?', [offset], (err, rows) => {
      if (err) return reject(err);
      resolve(rows);
    });
  });
  ...
  for (let i = 0; i < users.length; i += 2) {
    const row = [];
    const username1 = await getUsernameById(users[i].user_id);
    row.push({ text: username1 || users[i].user_id, callback_data: `reduce_saldo_${users[i].id}` });
    if (i + 1 < users.length) {
      const username2 = await getUsernameById(users[i + 1].user_id);
      row.push({ text: username2 || users[i + 1].user_id, callback_data: `reduce_saldo_${users[i + 1].id}` });
    }
    buttons.push(row);
  }
  ...
  if (page > 0) navigationButtons.push([{ text: '⬅️ Back', callback_data: `prev_reduce_${page - 1}` }]);
  if (offset + 20 < totalUsers) navigationButtons.push([{ text: '➡️ Next', callback_data: `next_reduce_${page + 1}` }]);

  replyMarkup.inline_keyboard.push(...navigationButtons);

  await ctx.reply('📉 *Pilih user untuk mengurangi saldo:*', { reply_markup: replyMarkup, parse_mode: 'Markdown' });
}

// Pagination Next / Back
bot.action(/next_reduce_(\d+)/, async (ctx) => {
  const page = parseInt(ctx.match[1]);
  await ctx.answerCbQuery();
  await sendReduceSaldoPage(ctx, page);
});

bot.action(/prev_reduce_(\d+)/, async (ctx) => {
  const page = parseInt(ctx.match[1]);
  await ctx.answerCbQuery();
  await sendReduceSaldoPage(ctx, page);
});
...
// ==========================
// 🔹 HANDLE INPUT NOMINAL KURANGI SALDO
// ==========================
bot.action(/reduce_saldo_(\d+)/, async (ctx) => {
  const userIdDb = ctx.match[1];
  const adminId = ctx.from.id;

  reduceSaldoState[adminId] = { step: 'reduce_saldo', targetUserId: userIdDb, saldo: '' };

  await ctx.answerCbQuery();
  await ctx.reply(
    `📉 Masukkan jumlah saldo yang ingin dikurangi untuk user ID *${userIdDb}:*`,
    { parse_mode: 'Markdown', reply_markup: { inline_keyboard: keyboard_nomor('reduce') } }
  );
});

// Keyboard angka
function keyboard_nomor(prefix = '') {
  const keys = [
    ['1','2','3'],
    ['4','5','6'],
    ['7','8','9'],
  ...
  return keys.map(row =>
    row.map(k => ({
      text: k === 'delete' ? '⌫' : k === 'confirm' ? '✅' : k,
      callback_data: prefix ? `${prefix}_${k}` : k
    }))
  );
}
...
bot.on('callback_query', async (ctx) => {
  const adminId = ctx.from.id;
  const state = reduceSaldoState[adminId];
  if (!state || state.step !== 'reduce_saldo') return;

  const data = ctx.callbackQuery.data.replace('reduce_', '');
  let currentSaldo = state.saldo || '';

  if (data === 'delete') {
    const newSaldo = currentSaldo.slice(0, -1);
    if (newSaldo === currentSaldo) return await ctx.answerCbQuery(); // tidak ada perubahan
    currentSaldo = newSaldo;
  } else if (data === 'confirm') {
    if (!currentSaldo) return await ctx.answerCbQuery('⚠️ Jumlah saldo tidak boleh kosong!', { show_alert: true });

    db.get('SELECT saldo FROM Users WHERE id = ?', [state.targetUserId], async (err, row) => {
      if (err || !row) return ctx.reply('❌ Gagal mengambil data user.');

      if (parseInt(row.saldo) < parseInt(currentSaldo)) {
        return ctx.reply('⚠️ Saldo user tidak cukup untuk dikurangi.');
      }

      db.run('UPDATE Users SET saldo = saldo - ? WHERE id = ?', [currentSaldo, state.targetUserId], async (err2) => {
        if (err2) return ctx.reply('❌ Gagal mengurangi saldo user.');
        ctx.reply(`✅ Saldo user berhasil dikurangi Rp${parseInt(currentSaldo).toLocaleString('id-ID')}`);
      });

      delete reduceSaldoState[adminId];
    });

    return;
  } else {
    if (!/^[0-9]+$/.test(data)) return await ctx.answerCbQuery('⚠️ Jumlah saldo tidak valid!', { show_alert: true });
    if (currentSaldo.length < 10) currentSaldo += data;
  }

  state.saldo = currentSaldo;
  const newMessage = `📉 Masukkan jumlah saldo yang ingin dikurangi:\n\nJumlah: *${currentSaldo}*`;

  // Edit message aman
  try {
    await ctx.editMessageText(newMessage, { reply_markup: { inline_keyboard: keyboard_nomor('reduce') }, parse_mode: 'Markdown' });
  } catch(err) {
