...
// === TRIAL SSH (NO BACKTICK ANYWHERE) ===
bot.action(/^trial_server_ssh_(\d+)$/, async (ctx) => {
  ...
  try {
    ...
    const [user, server] = await Promise.all([
      ...
      dbGetAsync('SELECT nama_server, domain, auth FROM Server WHERE id = ?', [serverId])
    ]);
    ...
    const maxTrial = role === 'admin' ? Infinity : role === 'reseller' ? 10 : 1;
    const maxShow = Number.isFinite(maxTrial) ? maxTrial : '∞';
    ...
    if (trialCount >= maxTrial) {
      return ctx.reply(`😅 Batas trial harian tercapai bro.\nLimit: ${maxShow}x per hari.`);
    }
    ...
    // === NORMAL DATA TANPA BACKTICK ===
    const username  = d.username || '-';
    const password  = d.password || '-';
    const expired   = d.expiration || '-';
    const ns        = d.ns_domain || '-';
    const city      = d.city || '-';
    const ipLimit   = d.ip_limit || '-';
    const pubkey    = d.public_key || '';

    const p = d.ports || {};
    const pick = (k, def) => p[k] || def;
...
TLS/SSL     : ${pick('ssl','443')}
HTTP        : ${pick('http','80, 8080')}
UDP SSH     : ${pick('udp_ssh','1-65535')}
Dropbear    : ${pick('dropbear','443, 109')}
WS SSH      : ${pick('ssh_ws','80, 8080')}
WS SSL      : ${pick('ssh_ssl_ws','443')}
OVPN SSL    : ${pick('ovpn_ssl','443')}
OVPN TCP    : ${pick('ovpn_tcp','1194')}
OVPN UDP    : ${pick('ovpn_udp','2200')}
DNS         : ${pick('dns','443, 53, 22')}
BadVPN UDP  : ${pick('badvpn','7100,7200,7300')}
────────────────────────────────────
IP Limit    : ${ipLimit}
────────────────────────────────────`;

    const openvpn = `https://${domain}:81/allovpn.zip`;
    const saveacc = `https://${domain}:81/ssh-${username}.txt`;

    // === FINAL MESSAGE ===
    let message = `<pre>${escapeHTML(box)}</pre>\n\n`;

    if (pubkey && pubkey !== 'Not Available') {
      message += `<b>🔒 PUBKEY</b>\n<pre>${escapeHTML(pubkey)}</pre>\n\n`;
    }

    message += `<b>🔗 LINKS</b>\n`;
    message += `OpenVPN : <a href="${openvpn}">${openvpn}</a>\n`;
    message += `SaveAcc : <a href="${saveacc}">${saveacc}</a>\n\n`;

    message += `<i>Trial otomatis hangus dalam 60 menit.</i>`;

    await ctx.reply(message, { parse_mode: 'HTML', disable_web_page_preview: true });

    await dbRunAsync('UPDATE users SET trial_count_today = trial_count_today + 1 WHERE user_id = ?', [userId]);
    await dbRunAsync('INSERT INTO trial_logs (user_id, username, jenis, created_at) VALUES (?, ?, ?, datetime("now"))',
      [userId, username, 'ssh']
    );

  } catch (err) {
    ctx.reply('❌ Terjadi error saat proses trial SSH.');
  }
});
// === TRIAL VMESS ===
