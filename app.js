...
async function sendMainMenu(ctx) {
  ...
  const text = `
<b>🚀 ${namaStore} CONTROL PANEL</b>
Halo <b>${firstName}</b>, selamat datang kembali! 👋
Sistem siap melayani transaksi Anda hari ini.

┌ 👤 <b>PROFIL PENGGUNA</b>
├ 🆔 <b>ID:</b> <code>${userId}</code>
├ 🔰 <b>Akses:</b> ${roleLabel}
├ 👥 <b>Total Member:</b> ${totalUserBot}
├ ⚡ <b>Transaksi:</b> ${totalAkunDibuat} Sukses
└ 💳 <b>Saldo Aktif:</b> Rp ${saldoFormatted}

<i>✨ Layanan otomatis aktif 24 jam.
Silakan pilih menu di bawah untuk memulai.</i>

<i>🕒 Data diperbarui: ${lastUpdate} WIB</i>
...
function generateServiceMenu(action) {
  ...
  if (action === 'create') {
    teks =
`<b>🛍️ BUAT AKUN PREMIUM SEKARANG!</b>

Dapatkan **VPN Super Cepat** dengan **Enkripsi Maksimal** (24/7). Akun langsung aktif **dalam hitungan detik** setelah pembayaran.

Pilih protokol andalan Anda di bawah ini:`;
    ...
    teks =
`<b>⏳ PERPANJANG AKUN - JAGA KONEKSI ANDA!</b>

Jangan biarkan koneksi Anda mati! Perpanjang sekarang untuk menjamin **stabilitas** dan **kecepatan** akun lama Anda.

Proses **Instan & Tanpa Setup Ulang**. Pilih protokol akun yang ingin diperpanjang:`;
    ...
    teks =
`<b>🎁 UJI COBA GRATIS - KUALITAS PREMIUM!</b>

Uji coba gratis selama **60 Menit** dengan kualitas server sama seperti premium. Buktikan sendiri performa terbaik kami!

Pilih salah satu protokol untuk klaim akun trial gratis Anda:`;
...
async function showTrialServerMenu(ctx, jenis) {
  try {
    ...
    const pesan =
`<b>✨ PILIH SERVER TRIAL ${jenisUpper} ANDA ✨</b>

Segera klaim **akses premium 60 menit** Anda!

Pilih server yang secara geografis **paling dekat** dengan Anda untuk memastikan kecepatan dan performa yang optimal:`;
...
bot.action(/^trial_server_ssh_(\d+)$/, async (ctx) => {
  ...
  try {
    ...
    const now = new Date();
    let diffDays = lastDate ? Math.floor((now - lastDate)/(1000*60*60*24)) : Infinity;

    if (diffDays >= 2) {
    trialCount = 0;
    await dbRunAsync('UPDATE users SET trial_count_today = 0, last_trial_date = date("now") WHERE user_id = ?', [userId]);
    }
...
✨ Selamat menggunakan layanan kami! ✨`;
  ...
  const headerText = '⏰ <b>TRIAL ACCOUNT SSH</b>';
...
🏷 <b>Trial By:</b> ${roleLabel.toUpperCase()} | ${trialCount + 1} dari ${maxShow}
...
bot.action(/^trial_server_vmess_(\d+)$/, async (ctx) => {
  ...
  try {
    ...
    if (diffDays >= 2) {
      trialCount = 0;
      await dbRunAsync(
        'UPDATE users SET trial_count_today = 0, last_trial_date = date("now") WHERE user_id = ?',
        [userId]
      );
    }
...
━━━━━━━━━━━━━━━━━━━━
👤 <b>User:</b> ${mention} (<code>${ctx.from.id}</code>)
🏷 <b>Trial By:</b> ${roleLabel.toUpperCase()} | ${trialKe} dari ${maxTrial === Infinity ? '∞' : maxTrial}
...
🌐 <b>Server:</b> ${namaServer}
⏳ <b>Duration:</b> 60 Minutes
🕒 <b>Time:</b> <b>${now.toLocaleString('id-ID',{timeZone:'Asia/Jakarta'})}</b>
━━━━━━━━━━━━━━━━━━━━
</blockquote>
`.trim();
      await bot.telegram.sendMessage(GROUP_ID, notifHtml, { parse_mode: 'HTML' });
...
`.trim();
    ...
    return bot.telegram.sendMessage(chatId,'❌ Terjadi kesalahan saat cek data trial TROJAN.');
  }
});
...
`.trim();
    ...
    return bot.telegram.sendMessage(chatId,'❌ Terjadi kesalahan saat cek data trial SHADOWSOCKS.');
  }
});
