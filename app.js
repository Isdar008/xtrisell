...
// === COMMAND: /cekmutasi (khusus admin) ===
bot.command("cekmutasi", async (ctx) => {
  const userId = ctx.from.id;
  const isAdmin = adminIds.includes(userId);

  if (!isAdmin) {
    return ctx.reply("🚫 Kamu tidak punya akses untuk perintah ini, bro.");
  }
  ...
  try {
    ...
    if (typeof res.data === "string") {
      ...
      text = "⚠️ Format data tidak dikenali (respon bukan teks).";
    }
