...
// 🔰 Kirim Menu Utama (HTML Version + Block Quote "<blockquote>")
async function sendMainMenu(ctx) {
  ...
  // Pastikan refreshCacheIfNeeded tersedia dan dipanggil
  if (typeof refreshCacheIfNeeded === 'function') {
  ...
  // --- Ambil Data ---
  let saldo = 0, role = '', reseller_level = '';
  let userTotal = 0;
  let topResellerText = '';
  let userHari = 0, userMinggu = 0, userBulan = 0; 
  let globalHari = 0, globalTotal = 0, globalMinggu = 0, globalBulan = 0; 
  let totalUserBot = 0;
  let usernameDisplay = `ID: ${userId}`; 

  try {
    // 1. Data user 
    const user = await dbGetAsync('SELECT saldo, role, reseller_level, username FROM users WHERE user_id = ?', [userId]);
    saldo = user?.saldo || 0;
    role = user?.role || 'user';
    reseller_level = user?.reseller_level || 'silver';
    usernameDisplay = user?.username ? `@${escapeHTML(user.username)}` : `ID: ${userId}`;

    // Total akun user
    const akunData = await dbGetAsync('SELECT COUNT(*) AS total FROM invoice_log WHERE user_id = ?', [userId]);
    userTotal = akunData?.total || 0;

    // Statistik akun user
    const userHariData = await dbGetAsync('SELECT COUNT(*) AS total FROM invoice_log WHERE user_id = ? AND DATE(created_at) = DATE("now")', [userId]);
    const userMingguData = await dbGetAsync('SELECT COUNT(*) AS total FROM invoice_log WHERE user_id = ? AND created_at >= datetime("now", "-7 days")', [userId]);
    const userBulanData = await dbGetAsync('SELECT COUNT(*) AS total FROM invoice_log WHERE user_id = ? AND created_at >= datetime("now", "start of month")', [userId]);
    
    userHari = userHariData?.total || 0;
    userMinggu = userMingguData?.total || 0;
    userBulan = userBulanData?.total || 0; 

    // Ambil Top 3 Creators Mingguan
    const topReseller = await dbAllAsync(`
      SELECT 
        u.username,
        i.user_id,
        COUNT(i.id) AS total_create
      FROM invoice_log i
      JOIN users u ON u.user_id = i.user_id
      WHERE i.created_at >= datetime('now', '-7 days') AND u.role = 'reseller'
      GROUP BY i.user_id
      ORDER BY total_create DESC
      LIMIT 3
    `);

    // FORMAT TOP CREATORS HORIZONTAL (Tetap dipertahankan untuk kerapian)
    if (topReseller.length > 0) {
      const medals = ['🥇', '🥈', '🥉'];
      
      const leaderboards = topReseller.map((r, i) => {
        const mention = r.username ? `@${escapeHTML(r.username)}` : `ID: ${r.user_id}`;
        // Format: 🥇 @User (5)
        return `${medals[i] || '🎖'} ${mention} (<b>${r.total_create}</b>)`;
      });

      // Dibuat dalam satu baris dengan header agar rapi
      topResellerText = `\n<b>🏆 Top Creators (7 Hari):</b>\n${leaderboards.join(' | ')}`;
    } else {
      topResellerText = '';
    }

    // Statistik global akun
    const globalHariData = await dbGetAsync('SELECT COUNT(*) AS total FROM invoice_log WHERE DATE(created_at) = DATE("now")');
    const globalTotalData = await dbGetAsync('SELECT COUNT(*) AS total FROM invoice_log');
    const globalMingguData = await dbGetAsync('SELECT COUNT(*) AS total FROM invoice_log WHERE created_at >= datetime("now", "-7 days")');
    const globalBulanData = await dbGetAsync('SELECT COUNT(*) AS total FROM invoice_log WHERE created_at >= datetime("now", "start of month")');
    
    const totalUserData = await dbGetAsync('SELECT COUNT(*) AS total FROM users');

    globalHari = globalHariData?.total || 0;
    globalTotal = globalTotalData?.total || 0;
    globalMinggu = globalMingguData?.total || 0; 
    globalBulan = globalBulanData?.total || 0; 
...
<blockquote>┌ Sistem otomatis & real-time  
├ Pembelian layanan VPN premium  
└ Akses internet cepat & aman</blockquote>
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
<b>👤 PROFIL PENGGUNA</b>
<blockquote>🏅 <b>Status</b>      : <code>${escapeHTML(roleLabel)} </code>
💳 <b>Saldo</b>       : <code>${formatRupiah(saldo)}</code>
🆔 <b>User ID</b>     : <code>${userId}</code>
📦 <b>Total Akun</b>  : <code>${userTotal} Akun</code>
🫂 <b>Total Pengguna</b>  : <code>${totalUserBot}</code></blockquote>
<b>🔔 BONUS JUMAT</b>
Setiap Hari Jumat, pengguna berhak menerima
bonus saldo otomatis setelah melakukan top up
dengan nominal minimum yang berlaku.

Nikmati keuntungan tambahan setiap minggunya.
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
${topResellerText ? `
<b>🏆 TOP CREATORS (7 HARI)</b>
<blockquote>${topResellerText.replace('\n<b>🏆 Top Creators (7 Hari):</b>\n','')}</blockquote>
` : ''}
<blockquote>📞 <b>Hubungi Admin:</b> ${escapeHTML(ADMIN_USERNAME)}</blockquote>
`.trim();

  // --- Tombol Keyboard (Tidak Berubah) ---
  const keyboard = [];
  const topButtons = [];
  const botAdminIds = global.adminIds || adminIds; 

  if (role === 'reseller') {
  topButtons.push({ text: '🏷️ Reseller Panel', callback_data: 'menu_reseller' });
}
if (role === 'admin' || botAdminIds.includes(String(userId))) {
  topButtons.push({ text: '🛡️ Admin Panel', callback_data: 'menu_adminreseller' });
}
if (topButtons.length) keyboard.push(topButtons);
...
if (role !== 'admin' && role !== 'reseller') {
  ...
  keyboard.push([
    { text: '⭐ Status Reseller', callback_data: 'reseller_status' },
  ]);
}
...
// ------- generateServiceMenu (update; returns HTML + keyboard) -------
function generateServiceMenu(action) {
  let keyboard = [], teks = '';

  if (action === 'create') {
    teks = `<b>🚀 Pembuatan Akun VPN Premium</b>\n<i>24/7 • Otomatis • Aman</i>\n\n` +
           `Pilih jenis akun di bawah, sistem akan memproses otomatis.`;
    ...
    teks = `<b>♻️ Perpanjangan Akun</b>\n<i>Pastikan saldo cukup & cek masa aktif akun sebelum perpanjang</i>\n\n` +
           `Pilih jenis akun yang ingin diperpanjang.`;
    ...
    teks = `<b>🧪 Akun Trial Gratis</b>\n<i>Durasi: 60 menit • Limit kuota • Satu trial per user</i>\n\n` +
           `Pilih jenis layanan untuk mencoba layanan kami.`;
...
// ------- showTrialServerMenu (update) -------
async function showTrialServerMenu(ctx, jenis) {
  try {
    const servers = await dbAllAsync('SELECT id, nama_server, lokasi, quota FROM Server');
    ...
    // keyboard: satu baris per server
    const keyboard = servers.map(s => [{
      text: `🌐 ${escapeHTML(s.nama_server)}`,
    ...
    // pesan ringkas & rapih
    const pesan = `<b>🧪 Pilih server untuk Trial ${escapeHTML(jenis.toUpperCase())}</b>\n\n` +
                  `<i>⚠️ Perhatian:</i>\n` +
                  `• Trial aktif 60 menit\n` +
                  `• Kuota trial terbatas\n` +
                  `• 1 user = 1 trial\n\n` +
                  `<b>Silakan pilih server:</b>`;
    ...
    await ctx.reply('❌ Terjadi kesalahan saat memuat daftar server.', { parse_mode: 'HTML' });
  }
}
  ...
  try {
    ...
    const serverCards = currentServers.map(s => {
      ...
      return (
        ...
        `• Kuota      : <code>${escapeHTML(String(s.quota || '-'))} GB</code>\n` +
...
bot.action('upgrade_to_reseller', async (ctx) => {
  ...
  const pesanKonfirmasi = 
    `🆙 𝗨𝗽𝗴𝗿𝗮𝗱𝗲 𝗸𝗲 𝗥𝗲𝘀𝗲𝗹𝗹𝗲𝗿\n\n` +
    `⚠️ Syarat: Memiliki saldo minimal Rp${minimumSaldo.toLocaleString('id-ID')} (saldo tidak akan dipotong)\n\n` +
    `🎯 Persyaratan lain:\n` +
...
bot.action('confirm_upgrade_reseller', async (ctx) => {
  ...
  await ctx.reply(
    escapeMarkdownV2(
      '🏆 𝗦𝗲𝗹𝗮𝗺𝗮𝘁! Kamu telah berhasil upgrade ke Reseller (Silver).\n' +
...
━━━━━━━━━━━━━━━━━━━━━━━
💌 *User:* ${mention}
⚠️ *Syarat:* Minimal saldo${escapeMarkdownV2(minimumSaldo.toLocaleString('id-ID'))}
...
🔽 Silakan pilih menu Reseller dibawah ini!!
`.trim();
...
bot.action(/^trial_server_ssh_(\d+)$/, async (ctx) => {
  ...
  // mention untuk Markdown (DM user) dan HTML (notif grup)
  const mentionMd = escapeMarkdown(rawName);
  const mentionHtml = escapeHTML(rawName);
  ...
  try {
    ...
    if (!server) {
      return bot.telegram.sendMessage(chatId, '❌ Server tidak ditemukan di database.');
    }

    const role = user?.role || 'user';
    let trialCount = user?.trial_count_today ?? 0;
    const lastDate = user?.last_trial_date ?? null;

    const maxTrial = role === 'reseller' ? 10 : role === 'admin' ? Infinity : 1;
    const maxTrialDisplay = Number.isFinite(maxTrial) ? String(maxTrial) : '∞';
    ...
    if (trialCount >= maxTrial) {
      return bot.telegram.sendMessage(
        chatId,
        `😅 Batas trial harian sudah tercapai bro. Kamu hanya bisa ambil *${maxTrialDisplay}x* per hari.`,
        { parse_mode: 'Markdown' }
      );
    }

    // --- Call API trial SSH di server tujuan ---
    const url = `http://${domain}:5888/trialssh?auth=${auth}`;

    let apiRes;
    try {
      apiRes = await axios.get(url);
    } catch (e) {
      logger.error('❌ Gagal call API trialssh:', e.message || e);
      return bot.telegram.sendMessage(
        chatId,
        '❌ Gagal menghubungi API trial SSH di server. Coba lagi nanti.'
      );
    }
    ...
    // --- Balasan ke user (Markdown, simpel & rapi) ---
    const replyText = `
⚡ *AKUN SSH TRIAL 60 MENIT*
...
🔗 *Link*
OpenVPN  : https://${domain}:81/allovpn.zip
Save Acc : https://${domain}:81/ssh-${username}.txt

📆 *Expired:* ${expired}
🛡️ *Limit IP:* \`${ip_limit}\`
(⏳ Trial 60 menit, auto hangus)
    `.trim();
...
👤 User: ${mentionHtml}
📩 Trial by: <b>${roleLabel}</b> | ${trialKe} dari ${maxTrialDisplay}
...
🕒 Waktu: <b>${new Date().toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' })}</b>
      ...
      try {
        ...
        logger.error('Gagal kirim notif grup SSH:', e.message);
      }
    }
  } catch (err) {
    logger.error('❌ Gagal proses trial SSH:', err);
    return bot.telegram.sendMessage(
      chatId,
      '❌ Terjadi kesalahan saat cek data trial SSH.'
    );
  }
});
...
bot.action(/^trial_server_vmess_(\d+)$/, async (ctx) => {
  ...
  try {
    // ==== Ambil data user untuk limit trial ====
    ...
    // Reset hitungan kalau hari sudah ganti
    if (lastDate !== today) {
      trialCount = 0;
      await dbRunAsync(
        'UPDATE users SET trial_count_today = 0, last_trial_date = ? WHERE user_id = ?',
    ...
    if (trialCount >= maxTrial) {
      return await bot.telegram.sendMessage(
        chatId,
        `😅 Batas trial harian sudah tercapai bro. Kamu hanya bisa ambil *${maxTrial}x* per hari.`,
        { parse_mode: 'Markdown' }
      );
    }

    // ==== Ambil data server (domain + auth) ====
    ...
    // ==== Panggil API trial di server tujuan ====
    const url = `http://${domain}:5888/trialvmess?auth=${auth}`;
    ...
    try {
      ...
      return bot.telegram.sendMessage(
        chatId,
        '❌ Gagal menghubungi API trial di server. Coba lagi nanti.'
      );
    }
    ...
    if (!apiRes.data || apiRes.data.status !== 'success') {
      ...
      return bot.telegram.sendMessage(
        chatId,
        `❌ Gagal membuat akun trial VMESS.\n\nDetail: ${msgErr}`
      );
    }

    // ==== Mapping data dari API ke variabel yang kita pakai ====
    const d = apiRes.data.data;
    ...
    const ns_domain  = d.ns_domain;
    const city       = d.city;
    const public_key = d.pubkey;           // nama di API: pubkey
    const expiration = d.expired;          // nama di API: expired
    const link_tls   = d.vmess_tls_link;
    const link_ntls  = d.vmess_nontls_link;
    const link_grpc  = d.vmess_grpc_link;

    // ==== Update hitungan trial user ====
    await dbRunAsync(
      'UPDATE users SET trial_count_today = trial_count_today + 1, last_trial_date = ? WHERE user_id = ?',
      [today, userId]
    );
...
`.trim();
    ...
    // ==== Notif ke group (kalau GROUP_ID di-set) ====
    if (GROUP_ID) {
      const notif = `
<blockquote>
🚀 <b>TRIAL ACCOUNT VMESS NEW</b>
👤 User: ${mention}
📩 Trial by: <b>${roleLabel}</b> | ${trialKe} dari ${maxTrial === Infinity ? '∞' : maxTrial}
🌐 Server: <b>${namaServer}</b>
🏪 Protocol: <b>VMESS</b>
⏳ Durasi: <b>60 Menit</b>
🕒 Waktu: <b>${new Date().toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' })}</b>
    ...
    logger.error('❌ Gagal proses trial VMESS:', err);
    return bot.telegram.sendMessage(
      chatId,
      '❌ Terjadi kesalahan saat proses trial VMESS. Coba lagi nanti.'
    );
  }
});
...
bot.action(/^trial_server_vless_(\d+)$/, async (ctx) => {
  ...
  try {
    ...
    // Ambil data user + server (sekaligus)
    const [user, server] = await Promise.all([
      dbGetAsync(
        'SELECT role, last_trial_date, trial_count_today FROM users WHERE user_id = ?',
        [userId]
    ...
    // Reset hitungan kalau hari berubah
    if (lastDate !== today) {
      trialCount = 0;
      await dbRunAsync(
        'UPDATE users SET trial_count_today = 0, last_trial_date = ? WHERE user_id = ?',
    ...
    if (trialCount >= maxTrial) {
      return await bot.telegram.sendMessage(
        chatId,
        `😅 Batas trial harian sudah tercapai bro.\nKamu hanya bisa ambil *${maxTrial}x* per hari.`,
        { parse_mode: 'Markdown' }
      );
    }

    // ==== Panggil API trial VLESS di server tujuan ====
    const url = `http://${domain}:5888/trialvless?auth=${auth}`;
    ...
    try {
      ...
      return bot.telegram.sendMessage(
        chatId,
        '❌ Gagal menghubungi API trial VLESS di server. Coba lagi nanti.'
      );
    }
    ...
    if (!apiRes.data || apiRes.data.status !== 'success') {
      ...
      return bot.telegram.sendMessage(
        chatId,
        `❌ Gagal membuat akun trial VLESS.\n\nDetail: ${msgErr}`
      );
    }
    ...
    const ns_domain  = d.ns_domain;
    const city       = d.city;
    const public_key = d.pubkey;        // nama field di API
    const expiration = d.expired;       // nama field di API
    const link_tls   = d.vless_tls_link;
    const link_ntls  = d.vless_nontls_link;
    const link_grpc  = d.vless_grpc_link;

    // ==== Update DB trial user ====
    await dbRunAsync(
      'UPDATE users SET trial_count_today = trial_count_today + 1, last_trial_date = ? WHERE user_id = ?',
      [today, userId]
    );
...
🕒 Waktu: <b>${new Date().toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' })}</b>
    ...
    logger.error('❌ Gagal proses trial VLESS:', err);
    return bot.telegram.sendMessage(
      chatId,
      '❌ Terjadi kesalahan saat proses trial VLESS. Coba lagi nanti.'
    );
  }
});
...
bot.action(/^trial_server_trojan_(\d+)$/, async (ctx) => {
  ...
  try {
    ...
    // Reset hitungan kalau hari ganti
    if (lastDate !== today) {
      trialCount = 0;
      await dbRunAsync(
        'UPDATE users SET trial_count_today = 0, last_trial_date = ? WHERE user_id = ?',
    ...
    // ==== Panggil API trial TROJAN di server ====
    const url = `http://${domain}:5888/trialtrojan?auth=${auth}`;
    ...
    try {
      ...
      return bot.telegram.sendMessage(
        chatId,
        '❌ Gagal menghubungi API trial TROJAN di server. Coba lagi nanti.'
      );
    }
    ...
    if (!apiRes.data || apiRes.data.status !== 'success') {
      ...
      return bot.telegram.sendMessage(
        chatId,
        `❌ Gagal membuat akun trial TROJAN.\n\nDetail: ${msgErr}`
      );
    }
    ...
    const username   = d.username;
    const uuid       = d.uuid;              // biasanya dipakai sebagai password trojan
    const ns_domain  = d.ns_domain;
    const city       = d.city;
    const public_key = d.pubkey;
    const expiration = d.expired;
    const link_tls   = d.trojan_tls_link;
    const link_grpc  = d.trojan_grpc_link;
...
🕒 Waktu: <b>${new Date().toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' })}</b>
    ...
    logger.error('❌ Gagal proses trial TROJAN:', err);
    return bot.telegram.sendMessage(
      chatId,
      '❌ Terjadi kesalahan saat cek data trial TROJAN.'
    );
  }
});
...
bot.action(/^trial_server_shadowsocks_(\d+)$/, async (ctx) => {
  ...
  try {
    ...
    // ==== Panggil API trial SHADOWSOCKS di server ====
    const url = `http://${domain}:5888/trialshadowsocks?auth=${auth}`;
    ...
    try {
      ...
      return bot.telegram.sendMessage(
        chatId,
        '❌ Gagal menghubungi API trial SHADOWSOCKS di server. Coba lagi nanti.'
      );
    }
    ...
    if (!apiRes.data || apiRes.data.status !== 'success') {
      ...
      return bot.telegram.sendMessage(
        chatId,
        `❌ Gagal membuat akun trial SHADOWSOCKS.\n\nDetail: ${msgErr}`
      );
    }
    ...
    const ns_domain  = d.ns_domain;
    const city       = d.city;
    const public_key = d.pubkey;          // field dari API
    const expiration = d.expired;         // field dari API
    const link_ws    = d.ss_link_ws;      // field dari API
    const link_grpc  = d.ss_link_grpc;    // field dari API
...
🕒 Waktu: <b>${new Date().toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' })}</b>
    ...
    logger.error('❌ Gagal proses trial SHADOWSOCKS:', err);
    return bot.telegram.sendMessage(
      chatId,
      '❌ Terjadi kesalahan saat cek data trial SHADOWSOCKS.'
    );
  }
});
    ...
    if (state.step.startsWith('exp_')) {
     ...
     `, [serverId]);
      ...
      const diskon = user.role === 'reseller'
        ? user.reseller_level === 'gold' ? 0.5
        : user.reseller_level === 'platinum' ? 0.7
        : 0.3
        : 0;
...
const actionLabel = action === 'renew' ? '♻️ Renew By' : '📩 Create By';
...
const invoiceHtml = `
<blockquote>
✅ <b>Transaksi Akun Berhasil</b>
👤 <b>User:</b> ${mention}
${actionLabel}: <b>${label}</b>
🔑 <b>Username:</b> <code>${username}</code>
📦 <b>Protocol:</b> <code>${type.toUpperCase()}</code>
🌐 <b>Server:</b> ${serverNama} | ${ipLimit} IP
💰 <b>Harga:</b> Rp ${hargaFinal.toLocaleString('id-ID')}
⏳ <b>Masa Aktif:</b> ${durasiHari} hari
${isReseller ? `📊 <b>Komisi:</b> Rp ${komisi?.toLocaleString('id-ID')}\n` : ''}🕒 <b>Waktu:</b> ${waktuSekarang}
</blockquote>
`.trim();
